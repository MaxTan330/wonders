 ...
 * @Author: MaxTan
 * @Description: 朝代表
 * @Date: 2021/09/08 17:09:41
 */
export type DynastyDic = {
  startYear: number; //开始年
  len: number; //时长
  countLen: number; //差值计算时长
  dynasty: string; //朝代
  miaohao: string; //庙号
  ruler: string; //统治者
  nianhao: string; //年号
 ...
 */
class Dynasty {
  private dicList: DynastyDic[];
  constructor() {
    this.dicList = this.getDicObj();
  }
  /**
   * 获取字典对象
   * @returns DynastyDic[]
   */
  getDicObj(): DynastyDic[] {
    const dicStr =
    ...
    const dicSplitArr = dicStr.split(',');
    const dicArr: DynastyDic[] = [];
    for (let i = 0; i < dicSplitArr.length; i += 7) {
      const dicObj: DynastyDic = {
        startYear: +dicSplitArr[i],
        len: +dicSplitArr[i + 1],
        countLen: +dicSplitArr[i + 2],
        dynasty: dicSplitArr[i + 3],
        miaohao: dicSplitArr[i + 4],
        ruler: dicSplitArr[i + 5],
        nianhao: dicSplitArr[i + 6],
      };
      dicArr.push(dicObj);
    }
    return dicArr;
  ...
  getNH(year: number): string {
    ...
    for (let i = 0; i < this.dicList.length; i++) {
      const item = this.dicList[i];
      const { startYear, countLen, dynasty, miaohao, ruler } = item;
      if (year < startYear || year >= startYear + item.len) {
        continue;
      }
      const countStr = `${item.nianhao}${year - startYear + 1 + countLen}年`;
      result += `${result ? ';' : ''}[${dynasty}] ${miaohao} ${ruler} ${countStr}`;
    }
    return result;
  }
}
export default new Dynasty();